...
class EuConnector {
    ...
    async _getJson(url, headers = {}, retried = false) {
        ...
        if (status >= 500 && typeof body === "string" && body.trimStart().startsWith("<")) {
            if (!retried) {
                console.log(`EU: AEM session expired at ${url}, re-logging in...`);
                this.loggedIn = false;
                await this.login();
                return this._getJson(url, headers, true);
            }
            throw new Error(`EU: AEM session error at ${url} (HTTP ${status})`);
        }
    ...
    async _tryGetIdentifier(vin, suffix) {
        ...
        try {
            ...
            const id = parsed.identifier || parsed.Identifier
                || (Array.isArray(parsed) ? parsed[0]?.identifier : null);
            if (id) console.log(`EU: found identifier via /metadata${suffix ? "/" + suffix : ""}: ${id}`);
            return id || null;
        } catch { return null; }
    }

    async getLatestData(vin) {
        // Try known metadata suffixes in order; pick first product that has real data
        const suffixes = ["partial", "full", ""];
        for (const suffix of suffixes) {
            const identifier = await this._tryGetIdentifier(vin, suffix);
            ...
            console.log(`EU [${suffix || "none"}]: ${datasets.length} ZIP(s) — ${available.length} with data, ${noContent.length} no_content_found`);
            if (latestAny) console.log(`EU [${suffix || "none"}]: newest: ${latestAny.name} (created: ${latestAny.createdOn})`);

            if (available.length) {
                const latest = available.sort((a, b) => new Date(b.createdOn) - new Date(a.createdOn))[0];
                console.log(`EU: downloading from product "${suffix || "none"}"...`);
                return this.downloadDataset(vin, identifier, latest.name);
            }
            console.log(`EU [${suffix || "none"}]: all empty, trying next product...`);
        }
        throw new Error("EU: no products with data found. Vehicle may not be transmitting live telemetry yet.");
    }
